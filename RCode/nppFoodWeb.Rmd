...
$$
\bar{H} = \frac{1}{S^*} \cdot \frac{ \sum_{j} H_j^0 \left( \frac{g_j}{m_j} - 1 \right) }{1 + \mu (S^* - 1)}
$$
...
Note that the second term involves a double sum. For simplification, assume symmetry and similar parameters among species.
...
But since the numerator is \( 1 - \mu \), which is the same as the coefficient on the left-hand side.
...
## Total Herbivore Biomass and NPP (additional)
...
_Left-hand side (LHS):_

$$
(1 - \mu) \sum_{i \in S} g_i H_i
$$

_Right-hand side (RHS):_
...
$$
(1 - \mu) \text{NPP} \approx \left( 1 + \mu (S^* - 1) \right) \text{NPP} \left[ 1 - \frac{ \mu S^* }{ 1 + \mu (S^* - 1) } \times \frac{1}{S^{*2}} \sum_{i,j \in S} \sqrt{ \frac{ m_i H_j^0 }{ m_j H_i^0 } } \right]
$$
...
The predation term acts as an additional mortality for herbivores.

## Net Primary Productivity with Predators

The NPP consumed by herbivores is:

$$
\text{NPP} = \sum_{i \in S} g_i H_i
$$

Predation redistributes energy from herbivores to predators but does not change the total NPP input into the system.
...
This adjustment ensures that the growth rates \( g_i \) are sufficient to balance both natural mortality and predation.
