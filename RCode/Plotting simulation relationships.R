...
ggplot(file1, aes(x = sigma, y = avg_shannon)) +
  geom_point() +
  labs(title = "Sigma vs Avg Shannon Index",
       x = "Sigma", y = "Avg Shannon Index") +
  theme_minimal()
...
ggplot(file1, aes(x = epsilon, y = avg_shannon)) +
  geom_point() +
  facet_wrap(~k_DA_name) +
  labs(title = "Average Shannon Vs Epsilon by Suitability Type",
