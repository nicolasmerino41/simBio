...
Understanding how small changes (perturbations) in species biomass propagate through the community helps us assess the system’s sensitivity and stability.
...
Herbivores that share predators can indirectly affect each other’s dynamics—a process (apparent competition). A linearized expression for the combined effect of a perturbation in herbivore \(j\) on herbivore \(i\) is:
$$
\delta K_i = (1-\mu)\,\delta H_i + \left(\varepsilon\,\nu^2\,N_{ij} + \mu\right)\,\delta H_j
$$
where:

- \(N_{ij}\) quantifies the number or strength of shared predators between herbivores \(i\) and \(j\).
