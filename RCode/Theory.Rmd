...
Multiply both sides by $(l + a H^*)$ to eliminate the denominator:

$$
\epsilon a \text{NPP} - d H^* (l + a H^*) = 0
$$

Simplify the right side:

$$
\epsilon a \text{NPP} - d l H^* - d a H^{*2} = 0
$$

Bring all terms to one side to form a quadratic equation in $H^*$:
...
$$
H^* = \frac{ - d l \pm \sqrt{ (d l)^2 - 4 d a (- \epsilon a \text{NPP}) } }{ 2 d a }
$$

Simplify the discriminant ($\Delta$):

$$
\Delta = (d l)^2 + 4 d a^2 \epsilon \text{NPP}
$$

Simplify numerator and denominator:

$$
H^* = \frac{ - d l \pm \sqrt{ (d l)^2 + 4 d a^2 \epsilon \text{NPP} } }{ 2 d a }
$$
...
### Selecting the Biologically Relevant Solution
...
- **Conservation and Management:**
  ...
  - **Enhancing $\text{NPP}$:** Practices that increase primary productivity (e.g., fertilization) can support higher herbivore populations.
  - **Reducing Mortality:** Protecting herbivores from predators or disease can increase their populations but may require managing resource availability to prevent depletion.
  - **Managing Leaching:** Reducing resource losses (e.g., through soil conservation) can increase both resource and herbivore biomass.

- **Ecosystem Stability:**

  - Understanding these dynamics helps predict how ecosystems respond to changes, such as climate shifts affecting $\text{NPP}$ or human activities altering mortality rates.

## Conclusion

The equilibrium analysis reveals the complex interplay between resource availability, herbivore consumption, and losses due to leaching and mortality. The positive root of the quadratic equation provides the biologically meaningful equilibrium herbivore biomass. By analyzing the parameters, we see that:

- **Resource input ($\text{NPP}$)** and **assimilation efficiency ($\epsilon$)** positively influence herbivore biomass.
- **Mortality rate ($d$)** and **leaching rate ($l$)** negatively influence herbivore biomass.
- **Attack rate ($a$)** enhances herbivore biomass by increasing consumption efficiency but can reduce resource biomass.

This model highlights the importance of balancing these factors to maintain ecosystem health and sustainability.
