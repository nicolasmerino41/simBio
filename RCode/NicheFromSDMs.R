...
for (species in species_names) {
  ...
  en <- raster(file_name)
    # ensemble(sdm_list[[species]], preds,
    #          filename = file_name,
    #          overwrite = TRUE, 
    #          setting = list(method = "weighted", stat = "auc"))
  cat("Ensemble", species, "done", "\n")
  # Evaluate the model
  ev <- sdm::evaluates(sdmData_list[[species]], en)
  th <- ev@threshold_based$threshold[2]
  
  # Extract probabilities and predictor variables
  probs <- extract(en, as(spanish_points, "Spatial"))
  bios5 <- extract(preds[[1]], as(spanish_points, "Spatial"))
  bios6 <- extract(preds[[2]], as(spanish_points, "Spatial"))
  bios12 <- extract(preds[[3]], as(spanish_points, "Spatial"))
  
  ################ ARMAND'S PART ######################
  thresholded_ensemble = en 
  thresholded_ensemble[]<-ifelse(thresholded_ensemble[] <=th | is.na(thresholded_ensemble[]),-0.01, thresholded_ensemble[])  
  ...
  mean_bio5 <- mean(val5$wc2.1_5m_bio_5, na.rm = T)
  sd_bio5 <- sd(val5$wc2.1_5m_bio_5, na.rm = T)
  val6 = extract(bio6_cropped, valid_coords)
  mean_bio6 <- mean(val6$wc2.1_5m_bio_6, na.rm = T)
  sd_bio6 <- sd(val6$wc2.1_5m_bio_6, na.rm = T)
  val12 = extract(bio12_cropped, valid_coords)
  mean_bio12 <- mean(val12$wc2.1_5m_bio_12, na.rm = T)
  sd_bio12 <- sd(val12$wc2.1_5m_bio_12, na.rm = T)
...
write.csv2(niches, "two_missing_species.csv", row.names = F)

# Making the rasters at 125x76
bio_prec = rast("C:/Users/MM-1/OneDrive/PhD/JuliaSimulation/simBio/Rasters/utm_raster_prec.tif")
bio5_cropped = crop(bio5, bio_prec)
bio6_cropped = crop(bio6, bio_prec)
bio12_cropped = crop(bio12, bio_prec)

bio5_cropped = terra::resample(bio5, bio_prec)
bio6_cropped = terra::resample(bio6, bio_prec)
bio12_cropped = terra::resample(bio12, bio_prec)

bio5_cropped[is.na(bio_prec)] <- NA
bio6_cropped[is.na(bio_prec)] <- NA
bio12_cropped[is.na(bio_prec)] <- NA

plot(bio5_cropped)

writeRaster(bio5_cropped, "C:/Users/MM-1/OneDrive/PhD/JuliaSimulation/simBio/Rasters/bio5.tif")
writeRaster(bio6_cropped, "C:/Users/MM-1/OneDrive/PhD/JuliaSimulation/simBio/Rasters/bio6.tif")
writeRaster(bio12_cropped, "C:/Users/MM-1/OneDrive/PhD/JuliaSimulation/simBio/Rasters/bio12.tif")
