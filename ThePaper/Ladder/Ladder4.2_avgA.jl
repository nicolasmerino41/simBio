...
# -----------------------------------------------------------
function rppp(
    S_vals, conn_vals, delta_vals, IS_vals;
    max_RC_steps = 10,
    ladder_steps = 1:12,
    Niter        = 10,
    tspan        = (0.0,500.0),
                        ...
                        try
                            ...
                            for i in (R+1):total, j in 1:total
                                if i !=j && rand() < conn && iszero(A_adj[i,j])
                                    A_adj[i,j]= rand(Normal(0.0, IS))
                                    A_adj[j,i]= -abs(rand(Normal(0, IS)))
                                end
                            end
                            # CAP EACH CONSUMERS TOTAL INTAKE TO PREVENT OVER‑ADVANTAGE OF GENERALISTS
                            max_intake = IS*5  # or any other upper‐bound you prefer
                            for i in (R+1):total
                                # compute sum of positive (prey) links
                                s = sum(A_adj[i,j] for j in 1:total if A_adj[i,j] > 0; init=0.0)
                                if s > max_intake
                                    factor = max_intake / s
                                    A_adj[i, :] .*= factor
                                    A_adj[:, i] .*= factor 
                            ...
                            while (any(isnan, xi_cons) || any(isnan, r_res)) && tries < max_calib
                                ...
                                for i in (R+1):total, j in 1:total
                                    if i !=j && rand() < conn && iszero(A_adj[i,j])
                                        A_adj[i,j]= rand(Normal(0.0, IS_reduction))
                                        A_adj[j,i]= -abs(rand(Normal(0, IS_reduction)))
                                    end
                                end
                                
                                # CAP EACH CONSUMER TOTAL INTAKE TO PREVENT OVER‑ADVANTAGE OF GENERALISTS
                                max_intake = IS_reduction*5  # or any other upper‐bound you prefer
                                for i in (R+1):total
                                    # compute sum of positive (prey) links
                                    s = sum(A_adj[i,j] for j in 1:total if A_adj[i,j] > 0; init=0.0)
                                    if s > max_intake
...
begin
    ...
    new_results11 = rppp(
        S_val, connectance_list, delta_list, IS_vals;
        max_RC_steps = 3,
        ladder_steps=1:16,
        Niter=1,
        tspan=(0.0, 500.0),
...
serialize("Results/new_results11.jls", new_results11)
CSV.write("Results/new_results11.csv", new_results11)