...
function ComputingLadder(
    ...
    @threads for (conn,IS,scen,delta,eps,m_val,g_val) in sample(combos, min(length(combos), number_of_combinations); replace=false)

        # 1) build A & epsilon
        local A = make_A(zeros(S,S),R,conn,scen; B_term = B_term).*IS
        local epsilon = clamp.(randn(S,S).*eps,0,1)
