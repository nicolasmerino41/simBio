...
function ComputingLadder(
    ...
    @threads for (conn,IS,scen,delta,epsi,m_val,g_val,ite) in sample(combos, min(length(combos), number_of_combinations); replace=false)
        ...
        # 1) build A & epsilon
        local A = make_A(zeros(S,S),R,conn,scen; B_term = B_term).*IS
        ...
        before_persistence_S = Dict(i => NaN for i in 1:20)
        after_persistence_S  = Dict(i => NaN for i in 1:20)
        rt_press_S   = Dict(i => NaN for i in 1:20)
        rt_pulse_S   = Dict(i => NaN for i in 1:20)
        collectivity_S = Dict(i => NaN for i in 1:20)
        resilience_S  = Dict(i=>NaN for i in 1:20)
        reactivity_S  = Dict(i=>NaN for i in 1:20)
        stable_S    = Dict(i=>NaN for i in 1:20)
        # Rmed_s    = Dict(i=>NaN for i in 1:20)
        # tau_S = Dict(i => Float64[] for i in 1:20)
        K_Xi_S = Dict(i => Float64[] for i in 1:20)
        @info "Running ladder"
...
A = ComputingLadder(
    ...
    growth_vals=[0.5, 1.0, 3.0, 5.0, 7.0],
    tspan=(0.,500.), tpert=250.0,
    number_of_combinations = 15,
    B_term = false,
    iterations=2
)
@info "we reached here"
serialize("Truth_2000_newXiK_plusS17.jls", A)