...
function survives!(fixed, p; tspan=(0.,500.), cb)
    ...
    sol  = solve(prob, AutoTsit5(Rosenbrock23()); callback=cb, abstol=1e-8, reltol=1e-8)
...
function NF_ShortLadder(
    ...
    @threads for (conn,IS,scen,delta,epsi,m_val,g_val,ite) in sample(combos, min(length(combos), number_of_combinations); replace=false)
        ...
        # 5a) Recompute xi_cons
        xi_cons = zeros(C)
        for k in 1:C
            i = R + k
            # 1) feeding gains (A>0)
            gain = sum(epsilon[i,j]*A[i,j]*R_eq_full[j] for j in 1:R if A[i,j] > 0; init=0.0 )
            # 2) predation losses: A_s[i,j]<0, but we need "+(-A)B"
            loss = sum(A[i,j]*C_eq_full[j-R] for j in R+1:S if A[i,j] < 0; init=0.0 )
            # consumer eq: xi = B_i - gain - loss
            xi_cons[k] = -C_eq_full[k] + gain + loss
        end
        ...
        for i in 1:R
            ...
            drain = sum(A[i,j]*C_eq_full[j-R] for j in R+1:S if A[i,j] < 0; init=0.0)
            # K_i = B_i + ? A[i,j] B_j
            K_res[i] = abs(-R_eq_full[i] + drain)
        end
        # if any(xi_hat .< 0.0) || any(K_hat .< 0.0)
        #     error("Xi or K out of bounds in step $step")
        # end
        K_res = max.(K_res, 1e-6)
        xi_cons = max.(xi_cons, 1e-6)
        ...
        # println("The iniial abundances after cleaning are: \n R_B0=$(B0[1:R]) \n C_B0=$(B0[R+1:S])")
        # println("In step0: \n xi_cons=$xi_cons \n K_res=$K_res")
        ...
        D, M = compute_jacobian(B0, p) # Notice here I'm using B0 as opposed to fixed like in the past
        # error("The error is line 126")
        J_full = D * M
        # error("The error is line 129")
        # !is_locally_stable(J_full) && continue
        # println("J_full is stable")
        # error()
        # resilience_full is -max Re(eig(J)), but compute_resilience returns max Re
        old_epsilon = copy(epsilon)
        resilience_full = compute_resilience(B0, p)
        @assert all(old_epsilon .== epsilon) "epsilon was mutated inside compute_resilience!"
        reactivity_full =  compute_reactivity(B0, p)
        ...
        # Rmed_full = median_return_rate(J_full, B0; t=t0, n=Rmed_iterations)
        # ssp_rmed_full = species_return_rates(J_full, B0; t=t0, n=Rmed_iterations)
        analytical_rmed_full = analytical_species_return_rates(J_full; t=0.001)
        ############### TRYING SOMETHING NEW ################
        prob1 = ODEProblem(trophic_ode!, B0, (tspan[1], tpert), p)
        sol1 = solve(prob1, AutoTsit5(Rosenbrock23()); callback = cb, reltol=1e-8, abstol=1e-8)
        ...
        for step in 4:11
            ...
            if step == 4
                ...
                rt_press2, _, _, before_s, after_s, _, _ = simulate_press_perturbation(
                B0, p_s, tspan, tpert, delta;
                solver=AutoTsit5(Rosenbrock23()), plot=false, cb=cb,
                full_or_simple=false
                )
                rt_pulse3, _, after_pulse3, _, _ = simulate_pulse_perturbation(
                B0, p_s, tspan, tpert, delta;
