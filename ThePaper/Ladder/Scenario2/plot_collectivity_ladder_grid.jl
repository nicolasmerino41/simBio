...
function plot_short_collectivity_ladder_grid(
    ncases::Int=16;
    S::Int=50, C::Int=20,
    conn_vals=[0.05,0.1,0.2,0.4],
    IS_vals=[0.1,1.0,2.0, 3.0],
    ...
    for (i, (conn, IS, scen, eps_scale)) in enumerate(picked)
        ...
        for step in 0:3
            if step == 0
                ...
                Aₛ, εₛ = short_transform_for_ladder_step(step, A_full, ε_full)
                push!(φs, compute_collectivity(Aₛ, εₛ))
            end
        end
        ...
        lines!(ax, 0:3, φs; color = :blue)
        scatter!(ax, 0:3, φs; color = :blue, markersize=6)
        # dashed line showing φ_full
        hlines!(ax, [φ_full]; linestyle = :dash, color = :black)
    end
...
# Call it!
plot_short_collectivity_ladder_grid()
