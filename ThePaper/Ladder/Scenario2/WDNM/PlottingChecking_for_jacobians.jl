...
function plot_scalar_correlations_D(
    G::DataFrame;
    scenarios = [:ER, :PL, :MOD],
    metrics = [
    ...
    step_names = ["Diagonal", "Off-diagonal", "All", "All mixing groups"]
...
function plot_vector_correlations_D(
    df::DataFrame;
...
for i in [0.1, 1.0]
    P = filter(row -> row[:IS] == i, G)
    println("subset size: ", nrow(P))
