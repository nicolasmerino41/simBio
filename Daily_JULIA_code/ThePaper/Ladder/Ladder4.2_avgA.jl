                    ...
                    try
                        ...
                        for i in (R+1):total, j in 1:total
                            if i !=j && rand() < conn && iszero(A_adj[i,j])
                                A_adj[i,j]= rand(Normal(0.02, delta)) * IS_reduction
                                A_adj[j,i]= -rand() * IS_reduction
                            end
                        end
                        # ————————————————
                        # CAP EACH CONSUMER'S TOTAL INTAKE TO PREVENT OVER‑ADVANTAGE OF GENERALISTS
                        max_intake = IS_reduction  # or any other upper‐bound you prefer
                        for i in (R+1):total
                            # compute sum of positive (prey) links
                            s = sum(A_adj[i,j] for j in 1:total if A_adj[i,j] > 0; init=0.0)
                            if s > max_intake
                                # scale down all of i’s interactions so they sum to max_intake
                                factor = max_intake / s
                                A_adj[i, :] .*= factor
                                A_adj[:, i] .*= factor  # also scale the reciprocal “loss” entries
                            end
                        end
                        # ————————————————
