...
full_survival_cells = filter(row -> row.survival_rate == 1.0, best_params)

# Initialize an empty list to store results for each cell
merged_results_list = []

# Iterate over all full survival cells
for i in 1:nrow(full_survival_cells)
    cell = full_survival_cells[i, :cell_id]
    begin
        ###############################
        # 1) Fixed Parameter Configuration
        ###############################
        mu_val           = full_survival_cells[i, :mu]
        mu_predation_val = full_survival_cells[i, :mu_predation]
        epsilon_val      = full_survival_cells[i, :epsilon_val]
    ...
    for (i_sp, sp) in enumerate(predator_list_full)
        ...
        push!(merged_results_list, merged_df)

        @info "Merged removal and network metrics for cell $cell."
    end
end

# Final list `merged_results_list` contains merged_df for each cell.
@info "All cells processed. Total cells: $(length(merged_results_list))"

AAA = merged_results_list[30]

#= PLOTING THE DIFFERENCES IN BIOMASS BETWEEN FULL SURVIVAL AND A KEYSTON SPECIES REMOVED =#
begin
    
    # Filter rows and extract data
    row_full = filter(row -> row.sp_removed == "none", AAA)
    key_species = findmin(AAA.survival_rate)[2]
    row_key = filter(row -> row.sp_removed == AAA.sp_removed[key_species], AAA)
    
    # Extract vectors for the plots
    abundance1 = vcat(row_full.H_biomass_vector[1], row_full.P_biomass_vector[1])
    abundance2 = vcat(row_key.H_biomass_vector[1], row_key.P_biomass_vector[1])
    categories = AAA.sp_id_removed[2:end]
    spe_names = AAA.sp_removed[2:end]
    abundance_difference = vcat(row_key.H_full_minus_H[1], row_key.P_full_minus_P[1])
    
    # Create the figure
    fig = Figure(resolution = (1200, 600))
    
    # First plot: abundance1
    ax1 = Axis(fig[1, 1], title = "Abundance 1", xlabel = "Species", ylabel = "Values")
    barplot!(ax1, categories, abundance1)
    ax1.xticks = (categories, spe_names)
    ax1.xticklabelrotation = π/2.5
    ax1.xticklabelsize = 8
    
    # Second plot: abundance2
    ax2 = Axis(fig[1, 2], title = "Abundance 2", xlabel = "Species", ylabel = "Values")
    barplot!(ax2, categories, abundance2)
    ax2.xticks = (categories, spe_names)
    ax2.xticklabelrotation = π/2.5
    ax2.xticklabelsize = 8
    
    # Third plot: abundance_difference
    ax3 = Axis(fig[1, 3], title = "Abundance Difference", xlabel = "Species", ylabel = "Values")
    barplot!(ax3, categories, abundance_difference)
    ax3.xticks = (categories, spe_names)
    ax3.xticklabelrotation = π/2.5
    ax3.xticklabelsize = 8
